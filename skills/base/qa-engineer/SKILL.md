...
8. Choose the next role deliberately: route `QA Validation Summary` to `code-reviewer` when implementation changes, defects, coverage gaps, residual code/contract/permission risks, or an explicit review request remain. Route to `handoff-writer` only when review is not needed and the next action is packaging or context transfer.
9. Summarize coverage, gaps, blockers, residual risks, and next-role handoff.
...
When QA validates changed implementation, prefer `code-reviewer` as the next target if there is any meaningful residual risk around behavior, contracts, permissions, migrations, integrations, accessibility, or missing regression coverage. Do not use `handoff-writer` to bypass review when review findings are still expected.
