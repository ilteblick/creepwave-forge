...
7. Choose the next role deliberately: route to `qa-engineer` when observable behavior still needs validation, and route to `code-reviewer` when the backend diff, permission logic, migration, integration, concurrency, or test coverage needs review. Do not mark implementation work complete when review was requested or high-risk code paths changed.
8. Summarize behavior, contracts, data/migration impact, tests, risks, and handoff notes.
...
Use `code-reviewer` as the target role when the next action is reviewing the implementation, diff, changed contracts, permissions, migrations, integration behavior, concurrency handling, or missing tests. Use `handoff-writer` only for packaging or transfer, not as a substitute for review.
